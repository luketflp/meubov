...
/**
 * The calving form itself: records the parto AND registers the calf in the herd
 * in one go — a birth that left no animal behind would quietly shrink the herd
 * count.
 *
 * Breed and lot start on the dam's and can be overridden; the optional birth
 * weight becomes the calf's first weighing, dated the calving. It is shared by
 * the dam's ficha ({@link RegisterCalvingDialog}) and the Nascimentos screen,
 * which differ only in how the dam gets chosen and in the footer's left button.
 */
...
/** Fresh fields for a dam: today's date, her breed and her lot. */
export function createInitialFields(dam: Animal): CalvingFields {
  return {
    date: todayISO(),
    calfEarTag: "",
    calfSex: "",
...
interface CalvingFormProps {
  dam: Animal;
  /** Called after the calving is stored; the caller closes its dialog. */
  onRegistered: () => void;
  /** Footer button shown to the left of "Registrar" (cancel, back, …). */
  leadingAction?: ReactNode;
}

export function CalvingForm({ dam, onRegistered, leadingAction }: CalvingFormProps) {
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="grid gap-1.5">
          ...
          <Input
            ...
            onChange={(e) => setFields((f) => ({ ...f, calfWeightKg: e.target.value }))}
            className="min-h-11 font-mono"
          />
